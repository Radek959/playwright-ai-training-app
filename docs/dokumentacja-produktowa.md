...
- Dwie pary pól warunkowych są zawsze wysyłane razem, bo API waliduje **scalone** zadanie: zmiana `taskType` z `"bug"` na inny wysyła dodatkowo `severity: null`, a odznaczenie „Requires manager approval” wysyła `{"requiresApproval": false, "approver": null}`.
